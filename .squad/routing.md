...
|-----------|----------|----------|
| Architecture, decisions, scope | Spike | System design, library choices, what to build next |
| HTTP, scraping, download engine | Jet | HTTP client, manga scrapers, file I/O, CLI args |
| Terminal UI, navigation, display | Faye | TUI layout, keyboard input, search, reading view |
| Tests, quality, edge cases | Ed | Writing tests, finding bugs, regression coverage |
| Code review | Spike | Review PRs, check quality, suggest improvements |
| Cross-cutting feature | Spike + Jet + Faye | Spawn all relevant agents in parallel |
